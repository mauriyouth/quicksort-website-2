...
test('Kanban permissions, inheritance, attribution, isolation, and revocation', async () => {
  ...
  try {
    ...
    await db.exec(await readFile(new URL('../supabase/migrations/20260924222257_kanban_undo.sql', import.meta.url), 'utf8'));
    ...
    const undoToken = async () => (await db.query('select kanban_undo_token() as token')).rows[0].token;
    const undo = async token => db.query('select undo_kanban_action($1)', [token]);
    const deletionToken = await undoToken();
    await as(admin, 'https://candidate.quicksort.fr');
    await assert.rejects(undo(deletionToken));
    await as(outsider);
    await assert.rejects(undo(deletionToken));
    await as(admin);
    await undo(deletionToken);
    assert.ok((await rows('kanban_projects')).some(p => p.id === project.id));
    assert.ok((await rows('kanban_cards')).some(c => c.title === 'Nested card'));
    assert.ok((await rows('kanban_board_members')).some(m => m.board_id === sibling.id));
    assert.equal((await rows('kanban_boards')).find(b => b.id === sibling.id).creator_name, 'Admin Alex');
    await assert.rejects(undo(deletionToken), /no longer available/);
    assert.notEqual(await undoToken(), deletionToken, 'redo receives a fresh token');
    await undo(await undoToken());
    assert.ok(!(await rows('kanban_projects')).some(p => p.id === project.id), 'redo deletes the project again');
    await undo(await undoToken());
    assert.ok((await rows('kanban_projects')).some(p => p.id === project.id), 'undo remains available after redo');

    await as(candidate);
    const undoCard = await insert("insert into kanban_cards(board_id,column_id,title) values($1,$2,'Undo me') returning *", [sibling.id,siblingColumn.id]);
    await undo(await undoToken());
    assert.ok(!(await rows('kanban_cards')).some(c => c.id === undoCard.id), 'candidate can undo own creation');
    await undo(await undoToken());
    assert.ok((await rows('kanban_cards')).some(c => c.id === undoCard.id), 'candidate can redo own creation');
    await undo(await undoToken());
    const discardedRedo = await undoToken();
    const movable = (await rows('kanban_cards')).find(c => c.title === 'Nested card');
    const otherColumn = (await rows('kanban_columns')).find(c => c.board_id === sibling.id && c.id !== siblingColumn.id);
    await db.query('update kanban_cards set column_id=$1 where id=$2', [otherColumn.id,movable.id]);
    await undo(await undoToken());
    assert.equal((await rows('kanban_cards')).find(c => c.id === movable.id).column_id, siblingColumn.id);
    await db.query('update kanban_cards set column_id=$1 where id=$2', [otherColumn.id,movable.id]);
    await assert.rejects(undo(discardedRedo), /no longer available/);
    const moveToken = await undoToken();
    await as(admin);
    await db.query("update kanban_cards set title='Changed by someone else' where id=$1", [movable.id]);
    await as(candidate);
    await assert.rejects(undo(moveToken), /changed since/);
    await as(admin);
    await undo(await undoToken());
    assert.equal((await rows('kanban_cards')).find(c => c.id === movable.id).title, 'Nested card');
    await db.query('delete from kanban_cards where id=$1', [movable.id]);
    await undo(await undoToken());
    assert.equal((await rows('kanban_cards')).find(c => c.id === movable.id).created_by, admin);
    await as(candidate);
    const authored = await insert("insert into kanban_cards(board_id,column_id,title) values($1,$2,'Keep attribution') returning *", [sibling.id,siblingColumn.id]);
    await as(admin);
    await db.query('delete from kanban_cards where id=$1', [authored.id]);
    await undo(await undoToken());
    const restoredAuthor = (await rows('kanban_cards')).find(c => c.id === authored.id);
    assert.equal(restoredAuthor.created_by, candidate);
    assert.equal(restoredAuthor.creator_name, authored.creator_name);
    assert.deepEqual(restoredAuthor.created_at, authored.created_at);
    await db.query('delete from kanban_board_members where board_id=$1 and user_id=$2', [sibling.id,candidate]);
    await undo(await undoToken());
    assert.ok((await rows('kanban_board_members')).some(m => m.board_id === sibling.id && m.user_id === candidate));
    await as(candidate);
    await db.query('update kanban_cards set column_id=$1 where id=$2', [otherColumn.id,authored.id]);
    const revokedToken = await undoToken();
    await as(admin);
    await db.query('with revoked as (delete from kanban_project_members where project_id=$3 and user_id=$2) delete from kanban_board_members where board_id=$1 and user_id=$2', [sibling.id,candidate,project.id]);
    await as(candidate);
    await assert.rejects(undo(revokedToken), /permission/);
    await as(admin);
    await undo(await undoToken());
    const restoreDrafts = (await rows('kanban_columns')).filter(c => c.board_id === sibling.id).map(c => ({id:c.id,name:c.name}));
    await save(project.id, 'Undo board edit', [...restoreDrafts.map(c => ({...c,name:c.name+' renamed'})), {name:'Extra'}], sibling.id);
    await undo(await undoToken());
    assert.equal((await rows('kanban_boards')).find(b => b.id === sibling.id).name, sibling.name);
    assert.deepEqual((await rows('kanban_columns')).filter(c => c.board_id === sibling.id).map(c => c.name), restoreDrafts.map(c => c.name));
    const batch = (await db.query("insert into kanban_cards(board_id,column_id,title) values($1,$2,'Batch one'),($1,$2,'Batch two') returning id", [sibling.id,siblingColumn.id])).rows;
    await undo(await undoToken());
    assert.ok(!(await rows('kanban_cards')).some(c => batch.some(b => b.id === c.id)), 'batch undone as one action');
    const createdBoard = await save(project.id, 'Undo creation', [{name:'One'}, {name:'Two'}]);
    await undo(await undoToken());
    assert.ok(!(await rows('kanban_boards')).some(b => b.id === createdBoard));
    // A newly-created board with a later child cannot be removed by undo.
    const isolatedBoard = await save(project.id, 'Concurrent child', [{name:'Tasks'}]);
    const isolatedToken = await undoToken();
    await db.exec('reset role');
    await db.query("select set_config('test.actor', '', false)");
    await db.query("insert into kanban_columns(board_id,name) values($1,'Later column')", [isolatedBoard]);
    await as(admin);
    await assert.rejects(undo(isolatedToken), /added related work/);
    assert.ok((await rows('kanban_boards')).some(b => b.id === isolatedBoard));
    await db.query('delete from kanban_projects where id=$1', [project.id]);
