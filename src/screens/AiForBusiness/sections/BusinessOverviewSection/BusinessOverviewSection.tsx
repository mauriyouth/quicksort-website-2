...
export const BusinessOverviewSection = (): JSX.Element => {
  ...
  return (
    <section className="relative flex flex-col items-center gap-8 sm:gap-12 md:gap-16 w-full bg-ink">
      <SectionGridOverlay showCenterLine={false} />
      <div className="flex flex-col items-center justify-center gap-12 sm:gap-16 md:gap-20 px-0 py-12 sm:py-16 md:py-24 w-full bg-ink relative z-[1]">
        <div className="flex flex-col lg:flex-row items-center justify-center gap-8 sm:gap-12 lg:gap-[130px] max-w-screen-xl w-full px-4 sm:px-8">
          <div className="flex flex-col items-start gap-8 sm:gap-10 md:gap-12 flex-1 min-w-0 w-full lg:w-auto">
            <div className="flex flex-col items-start gap-3 w-full">
              <p className="font-mono uppercase tracking-[0.12em] text-svc-business text-xs sm:text-sm">
                From AI capability to everyday leverage
              </p>

              <h2 className="font-display font-[number:var(--display-md-semibold-font-weight)] text-seasalt text-2xl sm:text-3xl md:text-[length:var(--display-md-semibold-font-size)] tracking-[var(--display-md-semibold-letter-spacing)] leading-[1.2] md:leading-[var(--display-md-semibold-line-height)] [font-style:var(--display-md-semibold-font-style)]">
                ...
                <div key={index} className="flex items-start gap-4 w-full">
                  <div className="flex flex-col items-start gap-4 sm:gap-5 flex-1">
                    <div className="flex flex-col items-start gap-2 pt-2.5 pb-0 px-0 w-full">
                      <h3 className="font-text-xl-semibold font-[number:var(--text-xl-semibold-font-weight)] text-seasalt text-lg sm:text-xl md:text-[length:var(--text-xl-semibold-font-size)] tracking-[var(--text-xl-semibold-letter-spacing)] leading-[var(--text-xl-semibold-line-height)] [font-style:var(--text-xl-semibold-font-style)]">
                        {feature.title}
                      </h3>

                      <p className="font-text-md-regular font-[number:var(--text-md-regular-font-weight)] text-fg-inv-2 text-sm sm:text-[length:var(--text-md-regular-font-size)] tracking-[var(--text-md-regular-letter-spacing)] leading-[var(--text-md-regular-line-height)] [font-style:var(--text-md-regular-font-style)]">
                        {feature.description}
                      </p>
                    </div>
                  </div>
