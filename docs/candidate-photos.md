...
Before deploying: pnpm --filter @quicksort/candidate build and node --test tests/team-photos.test.mjs. The download tests require the private deployment inputs and check all 28 JPEG files, valid-session sharing, unauthenticated denial, attachment headers, and file size limits.
